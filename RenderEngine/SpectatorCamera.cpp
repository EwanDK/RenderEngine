...
SpectatorCamera::SpectatorCamera(Vector3 position, float yaw, float pitch) : m_position(position), m_yaw(yaw), m_pitch(pitch), m_flashlight({}){
    DisableCursor();
}
...
void SpectatorCamera::Update(const std::vector<Input::InputAction>& actions, float dt){
    ...
    for (const auto& a : actions){
        ...
        switch (a.action){
        case Input::ActionID::CamForward: move = move + flatForward;
            break;
        case Input::ActionID::CamBack: move = move - flatForward;
            break;
        case Input::ActionID::CamRight: move = move + flatRight;
            break;
        case Input::ActionID::CamLeft: move = move - flatRight;
            break;
        case Input::ActionID::CamUp: move.y += 1.0f;
            break;
        case Input::ActionID::CamDown: move.y -= 1.0f;
            break;
        default: break;
        }
    }
